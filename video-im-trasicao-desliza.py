#script - seleciona imagens, corta com base na primeira, cria um video com trasições desliza
...
output_video = 'video_com_transicoes_desliza.avi'
...
if not images:
    ...
    # Função para criar uma transição de deslizamento entre duas imagens
    def add_slide_transition(img1, img2, steps=2):
        for i in range(steps):
            alpha = i / float(steps)
            beta = 1.0 - alpha
            slide_frame = np.zeros_like(img1)
            slide_frame[:, :int(width * alpha)] = img2[:, :int(width * alpha)]
            slide_frame[:, int(width * alpha):] = img1[:, int(width * alpha):]
            video.write(slide_frame)
    ...
    for i in range(len(images) - 1):
        ...
        add_slide_transition(adjusted_frame, adjusted_next_frame)
        print(images[i])
